...
*   **Google Chrome or Brave Browser:** The application uses Selenium to automate a web browser. You'll need either Google Chrome or Brave Browser installed. **Safari is NOT supported, as Selenium is configured to use ChromeDriver.** The appropriate ChromeDriver for your operating system is automatically managed by `webdriver-manager`. If using Brave, its executable path must be configured in `config.json`.
...
*   `"brave_path"`: (Optional) If you use Brave browser, provide the full path to its executable.
    *   **Windows Example:** `C:\\Program Files\\BraveSoftware\\Brave-Browser\\Application\\brave.exe`
    *   **macOS Example:** `/Applications/Brave Browser.app/Contents/MacOS/Brave Browser`
    *   If left blank or invalid, the app will attempt to use a generic ChromeDriver installation via `webdriver-manager` for Google Chrome.
...
├── run_prayer_app.bat          # (Windows only) Example batch file for Windows Task Scheduler
